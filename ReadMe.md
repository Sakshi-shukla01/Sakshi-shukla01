<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=6366F1&height=120&section=header"/>

# Sakshi Shukla

### Full-Stack & AI Engineer

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&pause=1000&color=6366F1&center=true&vCenter=true&width=700&lines=CS+Undergrad+%40+Bharati+Vidyapeeth%2C+CGPA+9.80%2F10;Research+Intern+%40+ARDE%2C+DRDO;ML+%2B+Web+Engineering;Building+Production-Ready+Systems" alt="Typing SVG" />

<br/>

<a href="https://github.com/Sakshi-shukla01">
<img src="https://img.shields.io/badge/GitHub-Sakshi--shukla01-181717?style=for-the-badge&logo=github"/>
</a>

<a href="https://linkedin.com/in/sakshishukla1008">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://www.instagram.com/_.sakshi._.shukla.__/">
<img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
</a>

<a href="https://discord.gg/ywzvat84t">
<img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white"/>
</a>

<a href="mailto:sakshishukla1008@gmail.com">
<img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=Sakshi-shukla01&style=for-the-badge&color=6366f1&label=PROFILE+VIEWS"/>

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=6366F1&height=3&section=header"/>

## 👩‍💻 About Me

CS undergrad (**CGPA 9.80/10.00**, expected 2027) who likes fixing real
problems more than writing about them — from a Docker startup race condition
to production concurrency bugs.

- 🔬 Research Intern at **ARDE, DRDO** (Ministry of Defence)
- 💻 Ex Full-Stack Developer Intern at **Uptoskill**
- 🌱 Open Source Contributor, **GirlScript Summer of Code (GSSoC)**
- 🧠 Exploring **LLMs, LangChain & RAG**
- 💬 Chat with the community on **[Discord](https://discord.gg/ywzvat84t)**

<img src="https://capsule-render.vercel.app/api?type=rect&color=6366F1&height=3&section=header"/>

## 🎓 Education

<table>
<tr><td>

**Bharati Vidyapeeth College of Engineering, Pune**
B.Tech, Computer Science & Engineering
`Jul 2023 – Jun 2027 (Expected)` · **CGPA: 9.80 / 10.00 (98%)**

</td></tr>
</table>

<img src="https://capsule-render.vercel.app/api?type=rect&color=6366F1&height=3&section=header"/>

## 💼 Experience

<details open>
<summary><b>Click to collapse / expand timeline</b></summary>

<br/>

**🔬 Research Intern — ARDE, DRDO, Ministry of Defence** · `Jul 2026 – Present`
Paid research internship in the CSE discipline, supporting assigned technical
research problems within a structured, mentorship-driven government research
program.

**💻 Full Stack Developer Intern — Uptoskill (Remote)** · `Mar 2026 – May 2026`
- Built RESTful API endpoints (Node.js, Express.js, PostgreSQL) for an AI-powered LMS, and helped troubleshoot issues raised during sprint cycles in an agile SDLC.
- Contributed to a Python (FastAPI) microservice, working with JWT and Firebase OAuth.

**🌱 Open Source Contributor — GirlScript Summer of Code (GSSoC)** · `Apr 2026 – Jun 2026`
- Identified and fixed a Docker startup race condition and several frontend feature gaps, across JavaScript/TypeScript and Python codebases.
- Merged **4 PRs**, including a `BackToTop` component and the Docker race-condition fix.
- Collaborated with maintainers through structured code review cycles across a distributed contributor team.

</details>

<img src="https://capsule-render.vercel.app/api?type=rect&color=6366F1&height=3&section=header"/>

## 📡 Live GitHub Snapshot

<div align="center">

<img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Fusers%2FSakshi-shukla01&label=Public%20Repos&query=%24.public_repos&color=6366F1&style=for-the-badge&logo=github"/>
<img src="https://img.shields.io/github/followers/Sakshi-shukla01?style=for-the-badge&logo=github&label=Followers"/>
<img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Fusers%2FSakshi-shukla01&label=Following&query=%24.following&color=6366F1&style=for-the-badge&logo=github"/>

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=6366F1&height=3&section=header"/>

## 📊 Stats, Languages & Streaks

<table>
<tr>
<td width="50%" valign="top">

<img src="https://github-readme-stats.vercel.app/api?username=Sakshi-shukla01&show_icons=true&count_private=true&include_all_commits=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=6366F1&icon_color=6366F1&rank_icon=github"/>

</td>
<td width="50%" valign="top">

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Sakshi-shukla01&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=6366F1&langs_count=10"/>

</td>
</tr>
<tr>
<td width="50%" valign="top">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=Sakshi-shukla01&theme=tokyonight&hide_border=true&background=0D1117&ring=6366F1&fire=6366F1&currStreakLabel=6366F1"/>

</td>
<td width="50%" valign="top">

<img src="https://github-profile-trophy.vercel.app/?username=Sakshi-shukla01&theme=tokyonight&no-frame=true&column=3&margin-w=8&margin-h=8&row=2"/>

</td>
</tr>
</table>

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Sakshi-shukla01&theme=tokyo-night&hide_border=true&bg_color=0D1117&color=6366F1&line=6366F1&point=ffffff"/>

</div>

# 📌 Pinned Projects

<table>
<tr>

<td width="50%" valign="top">

## 🎧 SoundGuard — Real-Time Acoustic Predictive Maintenance

<a href="https://github.com/Sakshi-shukla01/soundguard">
<img src="https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge&logo=github"/>
</a>

Detects and classifies industrial machine faults from live audio using an
unsupervised autoencoder + CNN classifier, streamed in real time to a React
dashboard via FastAPI/WebSocket.

**Highlights**

- Autoencoder-based anomaly detection (trained on normal-only sound)
- Supervised CNN fault classifier
- Mel-spectrogram preprocessing pipeline
- Live mic → WebSocket streaming demo
- Dockerized, multi-service setup

`Python` `PyTorch` `FastAPI` `WebSocket` `React` `Docker`

</td>

<td width="50%" valign="top">

## ⚡ Grid Fault Localization & RCA Agent

<a href="https://github.com/Sakshi-shukla01/grid-fault-agent">
<img src="https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge&logo=github"/>
</a>
<a href="https://sakshi898-grid-fault-agent1.hf.space/docs">
<img src="https://img.shields.io/badge/Live%20API-6366F1?style=for-the-badge"/>
</a>

Python-based diagnostic agent that identifies and localizes grid faults
automatically, with a custom evaluation methodology to track diagnostic
accuracy over time — an OpenEnv-compatible RL environment across 3 difficulty
tiers.

**Highlights**

- Custom RL environment (reset / step / scenarios API)
- MongoDB-backed data pipeline + REST endpoints
- Containerized with Docker, deployed on Kubernetes
- Prometheus + Grafana monitoring

`Python` `FastAPI` `Docker` `Kubernetes` `MongoDB` `Prometheus/Grafana`

</td>

</tr>
<tr>

<td width="50%" valign="top">

## 🎬 QuickShow — SaaS Movie Ticket Booking Platform

<a href="https://github.com/Sakshi-shukla01/ticket-movie">
<img src="https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge&logo=github"/>
</a>

Full-stack SaaS movie booking platform built around concurrent seat
reservation and payment processing.

**Highlights**

- Fixed real seat-booking race conditions using Redis atomic operations
- 11 automated unit + integration tests across 2 suites (Jest/Supertest)
- Stripe Checkout + webhooks, Inngest background jobs, Clerk auth

`JavaScript` `React` `Node.js` `MongoDB` `Redis` `Docker` `Jest/Supertest`

</td>

<td width="50%" valign="top" align="center">

<br/>

**Want to see everything else?**

<a href="https://github.com/Sakshi-shukla01?tab=repositories">
<img src="https://img.shields.io/badge/All%20Repositories-6366F1?style=for-the-badge&logo=github"/>
</a>

<br/><br/>

<sub>UPI Fraud Detection, Interactive E-Learning Dashboard, and Virtual
Jewelry Try-On are still on the profile — just not pinned right now.</sub>

</td>

</tr>
</table>

<img src="https://capsule-render.vercel.app/api?type=rect&color=6366F1&height=3&section=header"/>

# 🛠️ Tech Stack

## 💻 Languages

<div align="center">

<img src="https://skillicons.dev/icons?i=py,java,js,ts&perline=8"/>

<br/>

<img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white"/>

</div>

---

## ⚙️ Backend & APIs

<div align="center">

<img src="https://skillicons.dev/icons?i=fastapi,flask,nodejs,express&perline=8"/>

</div>

`RESTful APIs` `JWT` `Firebase OAuth` `Webhooks` `Background Jobs` `Agile/Scrum` `SDLC`

---

## 🎨 Frontend

<div align="center">

<img src="https://skillicons.dev/icons?i=react,nextjs,html,css,tailwind&perline=8"/>

</div>

---

## 🗄️ Databases

<div align="center">

<img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,redis&perline=8"/>

</div>

---

## ☁️ Cloud & DevOps

<div align="center">

<img src="https://skillicons.dev/icons?i=aws,docker,kubernetes,githubactions&perline=8"/>

</div>

`Docker` `Kubernetes` `AWS S3` `GitHub Actions CI/CD` `Technical Documentation` `Troubleshooting & Support`

---

## 🧪 Testing

<div align="center">

<img src="https://skillicons.dev/icons?i=jest&perline=8"/>

</div>

`Jest` `Supertest` `Unit Testing` `Integration Testing`

<img src="https://capsule-render.vercel.app/api?type=rect&color=6366F1&height=3&section=header"/>

# 🏆 Achievements

<details>
<summary><b>Click to expand</b></summary>

<br/>

| Achievement | Details |
|---|---|
| 🌱 **GSSoC Open Source** | 4 PRs merged across JS/TS & Python — including a `BackToTop` component and a Docker race-condition fix |
| 🥇 **Smart India Hackathon** | Digital tourism platform for Jharkhand heritage sites · [Certificate](https://drive.google.com/file/d/1pmrNRFLxXOyRUCfudqhxgn7WI4o8a4of/view) |
| 🧠 **Tata Crucible Quiz** | Cleared Prelims of the national Business & Technology Quiz · [Certificate](https://drive.google.com/file/d/1uqCCD4jPy9BTCFwUJeuC7b8tD_nbx5en/view) |
| 🚀 **Young Turks — Naukri** | Recognized through career excellence and talent initiative · [Certificate](https://drive.google.com/file/d/1Wv5oQ2gnqLdabfNOnjmlh2j9IUnkUdYj/view) |

</details>

<br/>

# 📜 Certifications

<details>
<summary><b>View Certifications</b></summary>

<br/>

| Certification | Issuer | Year |
|---|---|---|
| **OCI Gen AI Professional** | Oracle Cloud Infrastructure | — |
| **OCI Foundations Associate** | Oracle Cloud Infrastructure | — |
| **Cloud Computing** | NPTEL | 2025 |
| **Database Management Systems** | NPTEL | 2024 |
| **Java Programming Proficiency** | IIT Bombay — Spoken Tutorial | — |

</details>

<img src="https://capsule-render.vercel.app/api?type=rect&color=6366F1&height=3&section=header"/>

# 📚 Currently Learning

<div align="center">

### 🧠 LLM Engineering

`LangChain` · `RAG` · `Vector Databases` · `Embeddings`

### ⚙️ Backend Engineering

`Distributed Systems` · `Caching` · `Message Queues` · `Scalable APIs`

### 🚀 MLOps

`MLflow` · `Docker` · `CI/CD` · `Model Deployment`

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=6366F1&height=100&section=footer"/>
